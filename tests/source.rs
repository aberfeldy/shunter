...
fn test_sink<T>() -> (Arc<Mutex<Vec<T>>>, impl FnMut(T) -> std::pin::Pin<Box<dyn std::future::Future<Output = ()> + Send>>)
where
    T: Send + 'static,
{
...
async fn it_runs_with_type_map() {
    let (out, sink) = test_sink();
    Source::new(vec![1, 3, 2, 4]).map(|x| x * 2).map(|x| x.to_string()).run(sink).await;
    assert_eq!(*out.lock().unwrap(), vec!["2", "6", "4", "8"]);
}
#[tokio::test]
